...
function plot_regressor(model::Neuron, inputs::AbstractMatrix{<:Real}, targets::AbstractVector{<:Real}; resolution::Integer=100)
    ...
    lb = minimum(inputs)
    ub = maximum(inputs)
    x_range = reshape(collect(lb:(ub - lb)/100:ub), 101, 1)
...
# plots loss (MSE) against iterations of model
# inputs: losses/iterations from train!
# outputs: losses on y-axis, iterations on x-axis
